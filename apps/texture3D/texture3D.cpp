...
display()
{
    ...
    // analyze volume
    HPMCbuildHistopyramidUsingTexture( hpmc_h, volume_tex, 0.0 /*sin(0.1*t)*/ );
    ...
    glLoadIdentity();
    glFrustum( -0.3*aspect_x, 0.3*aspect_x, -0.3*aspect_y, 0.3*aspect_y, 0.5, 3.0 );
...
std::string custom_vertex_shader =
        ...
        "    float diff = max( 0.1, dot( cn, vec3(0.0, 0.0, 1.0) ) );\n"    \
    ...
    for(int k=0; k<volume_size_z; k++) {
        for(int j=0; j<volume_size_y; j++) {
            for(int i=0; i<volume_size_x; i++) {
                float x = 2.0*i/volume_size_x - 1.0;
                float y = 2.0*j/volume_size_y - 1.0;
                float z = 2.0*k/volume_size_z - 1.0;
                float f = 1.0 - 16*x*y*z - 4*x*x - 4*y*y - 4*z*z;
                buffer[ (k*volume_size_y + j)*volume_size_x + i ] = f;
            }
        }
    }
