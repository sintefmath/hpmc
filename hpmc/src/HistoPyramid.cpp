...
HPMCHistoPyramid::fragmentSource( bool first ) const
{
    ...
    src += "//generated by HPMC.HistoPyramid.fragmentSource\n";
    if( gl30 ) {
    ...
    for(int i=0; i<4; i++) {
        src += "        dot( vec4(1.0), " +
                std::string(first?"floor":"") +
                "( texture2D( HPMC_histopyramid, " +
                (gl30?"texcoord":"gl_TexCoord[0].xy") +
                " + HPMC_delta." +
                std::string((i&1)==0?"x":"y") +
                std::string((i&2)==0?"x":"y") +
                " ) ) )" +
                (i<3?",\n":"\n");
    }
    src +=  "    );\n"
            "    " +
            std::string(gl30?"fragment":"gl_FragColor") +
            " = sums;\n"
            "}\n";
    return src;
}
