...
HPMCtriggerHistopyramidBuildPasses( struct HPMCHistoPyramid* h )
{
    ...
    glActiveTextureARB( GL_TEXTURE0_ARB + h->m_hp_build.m_tex_unit_1 );
    ...
    glUniform1f( h->m_hp_build.m_base.m_loc_threshold, h->m_threshold );

    // And trigger computation.
    glBindFramebufferEXT( GL_FRAMEBUFFER_EXT, h->m_histopyramid.m_fbos[0] );
    glViewport( 0, 0, h->m_histopyramid.m_size, h->m_histopyramid.m_size );
    HPMCrenderGPGPUQuad( h );

    // If HP is only 1x1 texels big, we are finished.
    if( h->m_histopyramid.m_size_l2 < 1 ) {
        return true;
    }
    ...
    glViewport( 0, 0, h->m_histopyramid.m_size/2, h->m_histopyramid.m_size/2 );
    HPMCrenderGPGPUQuad( h );

    // If HP is only 2x2 texels big, we are finished.
    if( h->m_histopyramid.m_size_l2 < 2 ) {
        return true;
    }
    ...
    for(GLsizei m=2; m<=h->m_histopyramid.m_size_l2; m++) {
        ...
        glViewport( 0, 0, 1<<(h->m_histopyramid.m_size_l2-m),
                          1<<(h->m_histopyramid.m_size_l2-m) );
        HPMCrenderGPGPUQuad( h );
    }
    ...
    glTexParameteri( GL_TEXTURE_2D,
                     GL_TEXTURE_BASE_LEVEL, h->m_histopyramid.m_size_l2 );
    glTexParameteri( GL_TEXTURE_2D,
                     GL_TEXTURE_MAX_LEVEL, h->m_histopyramid.m_size_l2 );
    glGetTexImage( GL_TEXTURE_2D,
                   h->m_histopyramid.m_size_l2,
                   GL_RGBA, GL_FLOAT, NULL );
    glBindBuffer( GL_PIXEL_PACK_BUFFER, 0 );
    h->m_histopyramid.m_top_count_updated = false;
